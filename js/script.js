var defaultUrl =
    localStorageGetItem("api-url") || "https://secure.judge0.com/standard";
...
function loadMessages() {
    $.ajax({
        url: `https://ide.codebulbs.com/public/ide/messages.json?${Date.now()}`,
        type: "GET",
        headers: {
...
function loadRandomLanguage() {
    ...
    $selectLanguage.dropdown(
        "set selected",
        values[Math.floor(Math.random() * $selectLanguage[0].length)]
    );
    apiUrl = resolveApiUrl($selectLanguage.val());
    insertTemplate();
}
